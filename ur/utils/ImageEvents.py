...
# amount of time for main thread to sleep for (in milliseconds) until _NUM_IMAGES have been saved
SLEEP_DURATION = 200
...
class ImageEventHandler(PySpin.ImageEventHandler):
    ...
    def __init__(self, cam, filename, num):
        """
    ...
    def OnImageEvent(self, image):
        ...
        if self._image_count < self._NUM_IMAGES:
            ...
            if image.IsIncomplete():
                print('Image incomplete with image status %i...' %
                ...
                if self.filename:
                    ...
                    filename = './' + self.filename + '/Image-%s-%i.jpg' % (
                        self.filename, self.num)

                else:  # if serial number is empty
                    filename = './Image-%i.jpg' % (self.num)
...
def configure_image_events(cam, filename, num):
    """
    ...
    try:
        ...
        image_event_handler = ImageEventHandler(cam, filename, num)
...
def wait_for_images(image_event_handler):
    ...
    try:
        ...
        while image_event_handler.get_image_count() < image_event_handler.get_max_images():
            print('\t//\n\t// Sleeping for %i ms. Grabbing images...' %
                  SLEEP_DURATION)
            print("im here")
...
def run_single_camera(cam, filename, num):
    """
    ...
    try:
        ...
        err, image_event_handler = configure_image_events(cam, filename, num)
        if not err:
            return err
...
def get_main(filename="test", num=1):
    """
    ...
    for i, cam in enumerate(cam_list):
        ...
        result &= run_single_camera(cam, filename, num)
